...
from kairos.infrastructure.parsers.kconfig import (
    KconfigParser,
    is_kconfig_menu_document,
)
...
def test_menu_contains_and_depends_on_relations() -> None:
    ...
    assert (
        len(menu_relations) == 3
    )  # Main->Networking, Networking->WIFI, Networking->POWER_SAVE
...
def test_symbol_span_has_kconfig_locator() -> None:
    ...
    symbol_span = next(
        s for s in result.spans if s.span_kind == SpanKind.KCONFIG_SYMBOL
    )
