...
_HELP_TEXT = """\
KAIROS Terminal Lineage Interface — help

COMMANDS
  :home                       recent local activity
  :artifacts [kind]           list ingested artifacts
  :search <query>             full-text search (aliases: :s)
...
KEYBINDINGS
  Ctrl+P   focus command line        Tab / Shift+Tab   cycle pane focus
  Ctrl+R   history search            /                 start a search
  Enter    run / inspect selection   w                 well selector
...
Every result in this interface carries a full citation: artifact id,
source path, exact locator, parser + version, and one of the four layers
above. Trace edges marked DERIVED are explicit deterministic rule matches
— never a semantic-similarity or embedding-based claim.

LOCAL / READ-ONLY
  No network access. No telemetry. No LLM, no embeddings. This interface
  cannot edit, delete, or move any registered source file, and cannot run
  ingest or doctor-repair. The only writes it can make are the same ones
  the CLI already exposes: adding a note, and activating/clearing a well.
